...
这个博客从一份设计稿到上线，用了一天。活儿主要是 AI 干的，但"让 AI 把活干对"这件事本身，成了最大的工程量。这篇如实记录过程，包括翻车。
...
配 Umami 统计时，我把登录密码当成「ID」发给了 AI。它没有继续被用于配置，也没有写入任何文件，但它已经出现在对话记录里。教训值得写下来：**给任何工具的凭证，只给它需要的那部分**——website ID 是公开的，密码不是。遇到同样情况，第一件事是改密码。
